...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int r = row; r < row + height; r++) {
    ...
    DMA[DMA_CHANNEL_3].cnt = DMA_ON | DMA_16 | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT | width;
  }
...
void drawFullScreenImageDMA(const u16 *image) {
  ...
  DMA[DMA_CHANNEL_3].cnt = (WIDTH * HEIGHT) | DMA_ON | DMA_16 | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT;
}
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  //UNUSED(image);
  for (int r = row; r < row + height; r++) {
    DMA[DMA_CHANNEL_3].src = image + ((r - row) * width);
    DMA[DMA_CHANNEL_3].dst = videoBuffer + OFFSET(r, col, WIDTH);
    DMA[DMA_CHANNEL_3].cnt = width | DMA_ON | DMA_16 | DMA_DESTINATION_INCREMENT | DMA_SOURCE_INCREMENT;
  }
}
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  ...
  for (int r = 0; r < height; r++) {
    ...
    DMA[DMA_CHANNEL_3].dst = videoBuffer + OFFSET(r+row, col, WIDTH);
    DMA[DMA_CHANNEL_3].cnt = width | DMA_ON | DMA_DESTINATION_INCREMENT | DMA_SOURCE_INCREMENT;
  }
}
